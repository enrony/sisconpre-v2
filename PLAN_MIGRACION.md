...
- [ ] `larastan` en verde: ~898 errores pre-existentes (deuda del legado — tipos de retorno de `rules()`, relaciones `PrestamosDias::Prestamo`, controladores scaffold que declaran `@return Response` y devuelven Inertia, etc.). `pint` ya está en verde. `.env.production.example` sin secretos.
