...
- [x] **Auto-import de element-plus** (`unplugin-vue-components` + `unplugin-auto-import` con `ElementPlusResolver`, `importStyle: 'css'`): solo los componentes/directivas usados entran al bundle, con su CSS por componente. `app.ts` ya no hace `app.use(ElementPlus)` ni registra los ~290 iconos (no se usaban); el locale `es` se fija con `<el-config-provider>` en el render raíz. **Entry `app.js`: 1.008 kB → 209 kB** (−79%); ya no aparece el aviso de "chunks > 500 kB". Los `ElMessageBox`/`ElNotification` se auto-importan (se quitaron los `import` explícitos de 11 componentes). `resources/js/{auto-imports,components}.d.ts` generados y versionados. De paso se tiparon bien los slots `#default="{ row }"` de `el-table` (cast a la fila concreta) y los retornos de `tagType` (unión de `el-tag`).
...
1. [ ] **Préstamos** — 🟡 **listado + asistente de alta hechos**
    ...
    - [x] `TipoPrestamo` — `pages/TipoPrestamo.vue` con el **CRUD genérico** (`MaestroCrud`): descripción + cantidad + select de frecuencia (`/tipo_prestamo/tables` → `TipoFrecuenciaPrestamoAll`), respeta `por_defecto` (sin borrar los 8 tipos base). **Verificado en navegador** (10 tipos) + feature test `test_maestro_crud_tipo_prestamo`. `Frecuencias` ya estaba hecho en §4 Maestros.
2. 🟡 **Informes de pago** — **listado hecho** (`pages/PaymentReport.vue` + `components/paymentReport/{PaymentReportTable,PaymentReportFilters}.vue` + `stores/paymentReport.ts`): `el-table` con datos reales (`GET /payment_report/records`), columnas id/registrado/cliente/destino/monto (`value_amount`)/nº cuotas/estado (`el-tag` desde `status_description.color`); filtros clientes/fecha/estado (`GET /PaymentReportsMovementsEstatu/tables`)/destino del pago; paginación. **Verificado en navegador** (16 informes) + smoke test.
    ...
    - [x] **Detalle de un informe** (`components/paymentReport/DetalleInformeModal.vue` + `store.abrirDetalle`): acción "Ver detalle" de la fila → `GET /payment_report/record/{id}` (ya existía; devuelve `{ Prestamos: [...] }` con `prestamos_dias` marcados `p_seleccionado`). Modal con resumen (cliente/destino/monto/estado) + `el-collapse` por préstamo informado (nº de cuotas seleccionadas) + grilla de cuotas con la seleccionada resaltada (anillo azul), color por estado (pagada/demorada/festivo/domingo). Port de `showCuotasSelected.vue`. **Verificado en navegador** (informe #14: 3 préstamos, 38 cuotas) + feature test `test_detalle_informe`.
    - [x] **Gestión multi-informe de un cliente** (`components/paymentReport/GestionClienteModal.vue` + `store.abrirGestion`/`procesarGestion`): acción "Gestionar informes del cliente" de la fila → `GET /payment_report/obtenerReportPaymentActivos/{cliente}` (informes en estatus 1/4). Tabla con todos los informes pendientes del cliente; por fila un `el-select` de estado destino (todos menos el actual, deshabilitado si `finish_estatus`), input de motivo cuando el estado lo exige, y botón **Procesar** → reusa `store.changeState` (`POST /payment_report/change_estatus_report`); al procesar actualiza el estado en la fila y al cerrar recarga el listado. Port de `Management.vue`. **Verificado en navegador** (cliente con 6 informes pendientes) + feature test `test_gestion_informes_cliente`.
    - [x] **Subida de soportes (imágenes)**: `lib/fileToSupport.ts` (archivo → `{name, extension, base64}`, valida jpg/jpeg/png ≤ 2 MB) + `components/paymentReport/SupportUpload.vue` (`v-model` de `SupportImage[]`, input file oculto + lista con "quitar"). Cableado en: **Informar un pago** (soporte por método, siempre disponible), **Cambio de estado** y **Gestión multi-informe** (cuando el estado destino tiene `soporte = 1`; ningún estado del catálogo actual lo exige, pero el UI ya está). Backend: `PaymentReportService::registerSupportChangeState` / `registerSupportPaymentMehods` — **arreglado el fatal de Flysystem 3** (`getDriver()->getAdapter()->getPathPrefix()`, línea muerta) y endurecidos los `count()` sobre claves ausentes. Discos `supports_change_estatus_report` y `supports_prestamo` añadidos a `config/filesystems.php` (local; en Fase 8 → S3 sin tocar código). **Feature test** `test_cambio_de_estado_con_soporte` (guarda el archivo en disco fake + fila en `payment_reports_support_movements`).
    - [ ] Pendiente menor: **preview/galería** de los soportes ya cargados (no hay endpoint que los liste ni los sirva; era `previewImagesReportPayment` en el legado).
    - [x] **Fase 9 (resuelto)**: los endpoints auxiliares transversales se movieron a **`routes/shared.php`** (grupo `['web','auth','verified']`, sin `permission:`): `/banks/tables`, `/franquicias/tables`, `/payment_methods/tables`, `/clientes/tables`, `/clientes/lista-clientes-json[-basic]`, `/prestamos/obtenerPrestamosActivos/{cliente}`. Se quitaron de sus archivos de módulo (`Bank.php`, `Franquicias.php`, `PaymentMethod.php`, `clientes.php`, `prestamos.php`). Los `tables` que solo consume su propio módulo (`prestamos/tables`, `PaymentReportsMovementsEstatu/tables`, y los de los maestros) se quedan gateados. Feature test `test_endpoints_auxiliares_no_gateados_por_modulo` (un usuario sin `banks.listar` ya puede leer `/banks/tables`).
...
4. 🟡 **Maestros** — **CRUD genérico** `components/maestros/{MaestroCrud,MaestroFormModal}.vue` + `types.ts`: config declarativa (columnas + campos text/number/switch/select), tabla + `el-pagination` + modal de alta/edición (`PUT /<recurso>`) + borrado con confirmación (`DELETE /<recurso>/{id}/{page}`), todo gateado por `can('<recurso>.registrar|editar|eliminar')`. Errores de validación de Inertia mostrados en el form.
    ...
    - **Maestros: 13/13 hechos** (`TipoPrestamo` incluido — su página vive en §1 pero usa el mismo `MaestroCrud`).
...
- [x] **Gating por acción en las rutas de escritura** (además del `permission:<recurso>.listar` que ya aplica el grupo de `bootstrap/app.php`): cada `PUT`/`POST` de alta/edición lleva `permission:<recurso>.registrar|<recurso>.editar` y cada `DELETE` lleva `permission:<recurso>.eliminar` (13 maestros + `action`/`modules`). `PUT /prestamos` → `prestamos.registrar`; `POST /payment_report` → `payment_report.registrar|gestionar-informe-de-pago`; `POST /payment_report/change_estatus_report` → `payment_report.gestionar-informe-de-pago|payment_report.editar`; `PUT /user/actualizaCliente` y `PUT|DELETE /clientes` → `clientes.registrar|editar` / `clientes.eliminar`. Complementa (no reemplaza) el `FormRequest::authorize()`. Feature test `test_rutas_de_escritura_exigen_permiso_de_accion`.
...
- [x] **i18n de las páginas de auth**: `pages/auth/{Login,Register,ForgotPassword,ResetPassword,VerifyEmail}.vue` traducidos (títulos de layout, labels, placeholders, botones, enlaces) + defaults en español de `PasskeyVerify.vue`. **Backend**: creados `lang/es/{auth,passwords,pagination,validation}.php` (antes NO había ningún archivo de idioma → `auth.failed`, `validation.required`, etc. se mostraban literales). `validation.php` completo en español + `attributes` con nombres legibles de los campos más usados.
- [x] Pruebas _stock_ del starter kit: apuntadas a `prestamos_gilen_test` (MySQL). **`php artisan test` 71/71.** El runner de despliegue (§12) y cualquier CI deben crear esa BD.
- [x] **`larastan` en verde** (level 7): `pint` y `phpstan` ambos pasan.
...
- [x] `https://prestamos.gilensoft.com/login` carga con estilos; `.env` → 403; assets `/build/*` 200 sin _mixed content_.
...
- [ ] **Pendiente:** smoke test logueado: login como `enrony@gmail.com`, revisar Préstamos / Informes de pago / Maestros / Roles.
    - [x] **Informes de pago lento → optimizado** (commits `209fe87` + `7d507ff`): `GET /payment_report/records` hacía `->get()` de todo + troceo en PHP + `prestamoFormated` (árbol préstamos→cuotas que la grilla no usa) y serializaba la columna `cliente` completa (blobs legados de hasta 2,8 MB). Medido con la data importada: ~150-200 queries / ~980 ms / 14,4 MB de JSON → **4 queries / ~27 ms / ~9 KB**. `lista()` pasa a builder paginado por SQL (columnas mínimas + `json_extract` del nombre del cliente + `withCount`/`withSum`); mismo shape de JSON, sin cambios de front. Nuevo `PaymentReportRecordsTest` fija forma + cotas de rendimiento.
    - [x] **Préstamos (`/prestamos`) → mismo tratamiento** (commit `78e6d12`): `POST /prestamos/records` tenía idéntico patrón `lista()->get()` + `paginate()` local, `select *` con los blobs `cliente`, filtros de rango `inicio`/`fin` rotos y `orderBy` sin whitelist. `lista()` pasa a builder paginado por SQL (`prestamos_dias`/`p_estatus` eager-loaded ya sólo de la página); se borran `paginate()` local e `index_old()` muerto. Con la data importada: 233 ms / 52 KB / toda la tabla + cuotas → **~27 ms / ~10 KB / sólo la página**. Nuevo `PrestamosRecordsTest`.
    - [ ] **En el server** (tras el `git pull` del release): correr `datos:compactar-clientes --dry-run` y luego sin flag — recorta las filas `cliente` infladas de `payment_reports` **y** `prestamos` (7 filas, ~7,2 MB).
